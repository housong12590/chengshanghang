...
class Printer {
    ...
    static void printJsonRequest(LoggingInterceptor.Builder builder, Request request) {
        ...
        logger.log(tag, REQUEST_UP_LINE);
        logLines(tag, new String[]{URL_TAG + request.url()}, builder.getLogger(), false);
        logLines(tag, getRequest(request, builder.getLevel()), builder.getLogger(), true);
        if (builder.getLevel() == Level.BASIC || builder.getLevel() == Level.BODY) {
            logLines(tag, requestBody.split(LINE_SEPARATOR), builder.getLogger(), true);
        }
        logger.log(tag, END_LINE);
    }
    ...
    static void printFileRequest(LoggingInterceptor.Builder builder, Request request) {
        ...
        logLines(tag, getRequest(request, builder.getLevel()), builder.getLogger(), true);
        if (builder.getLevel() == Level.BASIC || builder.getLevel() == Level.BODY) {
            logLines(tag, OMITTED_REQUEST, builder.getLogger(), true);
        }
        logger.log(tag, END_LINE);
    }
    ...
    static void printFileResponse(LoggingInterceptor.Builder builder, long chainMs, boolean isSuccessful,
        ...
        logLines(tag, getResponse(headers, chainMs, code, isSuccessful,
                builder.getLevel(), segments, message), builder.getLogger(), true);
        logLines(tag, OMITTED_RESPONSE, builder.getLogger(), true);
    ...
    private static void logLines(String tag, String[] lines, Logger logger, boolean withLineSize) {
        for (String line : lines) {
            ...
            for (int i = 0; i <= lineLength / MAX_LONG_SIZE; i++) {
                ...
                logger.log(tag, DEFAULT_LINE + line.substring(start, end));
            }
        }
    }
